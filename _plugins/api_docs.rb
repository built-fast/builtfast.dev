  ...
  class ApiDocsGenerator < Generator
    ...
    def normalize_endpoint(endpoint, group_name)
...

      {
        ...
        '_raw' => endpoint,
        'openapi_yaml' => build_openapi_yaml(endpoint, group_name, method, uri, url_params, query_params, body_params)
      }
    end
    ...
    def build_openapi_yaml(endpoint, group_name, method, uri, url_params, query_params, body_params)
      metadata = endpoint['metadata'] || {}
      ...
      parameters = build_openapi_parameters(url_params, query_params)
      if parameters.any?
      ...
      if %w[POST PUT PATCH].include?(method) && body_params.any?
        ...
        body_params.each do |param|
          yaml_lines.concat(build_openapi_property(param, 16))
        end
    ...
    def build_openapi_parameters(url_params, query_params)
      lines = []
      ...
      url_params.each do |param|
        ...
        lines << "          description: #{yaml_escape(param['description'])}" if param['description'] && param['description'] != ''
      end
    ...
    def build_openapi_property(param, indent)
      pad = ' ' * indent
      lines = []
      lines << "#{pad}#{param['name']}:"
      lines << "#{pad}  type: #{openapi_type(param['type'])}"
      lines << "#{pad}  description: #{yaml_escape(param['description'])}" if param['description'] && param['description'] != ''
      if param['enum_values'] && param['enum_values'].any?
        lines << "#{pad}  enum:"
